...
; 在无边框窗口中显示动画图像并返回句柄。详情见 ImagePutWindow
...
; 将图像按指定格式编码并返回 SafeArray COM 对象
;   extension  - 文件编码格式            | 字符串   ->  bmp、gif、jpg、png、tiff
;   quality    - JPEG 质量等级            | 整数     ->  0 - 100
ImagePutSafeArray(image, extension := "", quality := "") {
    return ImagePut("SafeArray", image, extension, quality)
}

; 将图像绘制到共享屏幕设备上下文并返回坐标数组
;   screenshot - 屏幕坐标  ->  [x,y,w,h] 或 [0,0]
;   alpha      - 透明替换颜色  ->  0xFFFFFF
...
; 在带边框的窗口中显示图像，并返回窗口句柄
;   title      - 窗口标题            | 字符串   ->  如"MyTitle"
;   pos        - 窗口坐标            | 数组     ->  [x,y,w,h]（指定位置和尺寸）或 [0,0]（自动定位）
;   style      - 窗口样式            | 无符号整数 ->  如WS_VISIBLE（可见窗口）
;   styleEx    - 窗口扩展样式        | 无符号整数 ->  如WS_EX_LAYERED（分层窗口）
;   parent     - 父窗口句柄          | 指针     ->  父窗口的hwnd
;   playback   - 是否播放动画？      | 布尔值   ->  True（启用动画播放）
;   cache      - 是否缓存动画帧？    | 布尔值   ->  False（不缓存，适用于节省内存）
...
class ImagePut {
    ...
    static possible(coimage) {
        ...
        if coimage.HasProp("prototype") && coimage.prototype.HasProp("__class") && coimage.prototype.__class ==
        "ClipboardAll"
...
array:
        ...
        && coimage[1] ~= "^-?\d+$" && coimage[2] ~= "^-?\d+$" && coimage[3] ~= "^(?!0+$)\d+$" && coimage[4] ~=
        "^(?!0+$)\d+$"
        && coimage[1] > -65536 && coimage[1] < 65536 && coimage[2] > -65536 && coimage[2] < 65536 && coimage[3] < 65536 &&
        coimage[4] < 65536
...
object:
        ...
        if coimage.HasProp("ptr") && coimage.HasProp("size") && coimage.size >= 24 && this.GetExtensionFromBuffer(
            coimage)
        ...
        or coimage.HasProp("size") && (coimage.HasProp("stride") || coimage.HasProp("width") || coimage.HasProp(
            "height")))
            return "Buffer"
...
string:
        ...
        if DllCall("CloseHandle", "ptr", DllCall("OpenFileMapping", "uint", 2, "int", 0, "str", "ImagePut_" coimage,
            "ptr"))
            return "SharedBuffer"
...
handle:
        ...
        if (DllCall("GetObjectType", "ptr", coimage, "uint") == 3 || DllCall("GetObjectType", "ptr", coimage, "uint") ==
        10)
            return "DC"
...
FrameAcquired:
    ...
    static WICBitmapToBitmap(image) {
        ...
        convert := 16 != DllCall("ntdll\RtlCompareMemory", "ptr", format, "ptr", GUID_WICPixelFormat32bppBGRA, "uptr",
            16)
